...
const createUser = async (req, res) => {
  try {
    ...
    // Gerando o hash Argon2id da senha antes de persistir
...
const loginUser = async (req, res) => {
  try {
    ...
    if (email != undefined) {
      ...
      if (user != undefined) {
        // Comparando a senha com o hash Argon2id salvo
        const correct = await argon2.verify(user.password, password)
        // Verificando se a senha é valida
        if (correct) {
            // Se a senha estiver correta, gera o TOKEN (48h, conforme Aula 05)
